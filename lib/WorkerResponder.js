...
WorkerResponder["prototype"]["getOrigin"]= WorkerResponder_getOrigin;// WorkerResponder#getOrigin():String
...
function _handleWorkerEvent(event) {
    if (event.type === "message") {
        ...
        if (head["INIT"]) {
            that._origin = head["ORIGIN"];
          //that._baseDir = this._origin.split("/").slice(0, -1).join("/") + "/",
...
function WorkerResponder_getOrigin() { // @arg String
    return this._origin;
}
