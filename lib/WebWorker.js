...
WebWorker["prototype"]["inbox"]   = WebWorker_inbox;   // WebWorker#inbox(data:Object, task:Task, id:String):void
...
function WebWorker_inbox(data, // @arg Object
                         task, // @arg Task
                         id) { // @arg String - Object ID
                               // @desc Message over WebWorker
    var requestData = {
            "command":  "inbox", // WorkerResponder#on("inbox", callback)
            "data":     data
        };

    this["request"](requestData, null, function(err, event) {
        if (!err) {
            task.set(id, event["data"]["result"] || "");
        }
        task.done(err);
    });
}
...
function _handleResponse(event) {
    ...
    if (workerID === that._workerID) {
        var property = "__RESPONSE__" in event["data"] ? "__RESPONSE__"
                                                       : "__REQUEST__";

        // hide event.data[__RESPONSE__ || __REQUEST__]
        Object.defineProperty(event.data, property, {
            configurable: true,
            enumerable: false,
            writable: false
        });
