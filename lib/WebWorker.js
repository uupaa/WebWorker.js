...
function WebWorker_inbox(task,   // @arg Task
    ...
    // http://twitter.com/uupaa/status/477428467009482752
/*
    Object["defineProperty"](data, "command", {
        "configurable": true,
        "enumerable": false,
        "writable": false,
        "value": "inbox"
 ...
 */
    if ("command" in data) {
        data["__COMMAND__"] = "inbox";
    } else {
        data["command"] = "inbox";
    }
    this["request"](data, null, function(err, event) {
        if (!err) {
            task["set"](name, event["data"]["result"] || "");
        }
        task["done"](err);
    });
}
...
function _createWorkerResource(that) {
    if (that._inline) {
        ...
        that._blobSource = _URL["createObjectURL"](blob);
    }
