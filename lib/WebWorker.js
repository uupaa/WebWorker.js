...
function WebWorker(param,      // @arg Object: { origin, inline, source, script, id, verbose }
                   ...
                   callback) { // @arg Function: callback(err:Error/null, event:EventObject):void
                               // @help: WebWorker
//{@assert
    _if(!Valid_type(param,        "Object",        "origin,inline,source,script,id,verbose"), "WebWorker(param)");
    _if(!Valid_type(param.origin, "String/omit"),  "WebWorker(param.origin)");
    _if(!Valid_type(param.inline, "Boolean/omit"), "WebWorker(param.inline)");
    _if(!Valid_type(param.source, "String"),       "WebWorker(param.source)");
    _if(!Valid_type(param.script, "Array/omit"),   "WebWorker(param.script)");
    _if(!Valid_type(param.id,     "Integer/omit"), "WebWorker(param.id)");
    _if(!Valid_type(param.verbose,"Boolean/omit"), "WebWorker(param.verbose)");
